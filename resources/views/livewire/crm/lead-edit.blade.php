<div>
    ...
    <div class="container mb-5">
        <div class="row">
                        ...
                        <form wire:submit.prevent="save">
                            <div class="row justify-content-between mb-2">
                                ...
                                <div class="">
                                    ...
                                    <input type="hidden" id="lead_id" class="form-control form-control-sm" value="{{ $lead->id }}" readonly>
                                </div>
                                
                                <!-- Lead Status -->
                                <div class="col-md-3">
                                    <label class="fw-bold small">Lead Status:</label>
                                    @if($lead_status_id)
                                        @php
                            ...
                            <div class="row g-2 mb-2">
                                ...
                                <div class="col-md-4">
                                    <label for="lead_status_id" class="form-label fw-bold small">Lead Status</label>
                                    <select id="lead_status_id" wire:model.live="lead_status_id" class="form-select form-select-sm">
                                        <option value="">Select Status</option>
                                        @foreach($leadStatuses as $status)
                                            <option value="{{ $status->id }}">{{ $status->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('lead_status_id') <span class="text-danger small">{{ $message }}</span> @enderror
                                </div>
                                
                                <!-- Segment -->
                                <div class="col-md-4">
                                    <label for="segment_id" class="form-label fw-bold small">Segment</label>
                                    <select id="segment_id" wire:model.live="segment_id" class="form-select form-select-sm">
                                        <option value="">Select Segment</option>
                                        @foreach($segments as $segment)
                                            <option value="{{ $segment->id }}">{{ $segment->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('segment_id') <span class="text-danger small">{{ $message }}</span> @enderror
                                </div>
                                 <!-- Sub-Segment -->
                                 <div class="col-md-4">
                                    <label for="sub_segment_id" class="form-label fw-bold small">Sub-Segment</label>
                                    ...
                                    @error('sub_segment_id') <span class="text-danger small">{{ $message }}</span> @enderror
                                </div>
                            </div>
                            ...
                            <div class="row g-2 mb-2">
                                ...
                                <!-- Lead Source -->
                                <div class="col-md-4">
                                    <label for="lead_source_id" class="form-label fw-bold small">Lead Source</label>
                                    <select id="lead_source_id" wire:model="lead_source_id" class="form-select form-select-sm">
                                        <option value="">Select Source</option>
                                        @foreach($leadSources as $source)
                                            <option value="{{ $source->id }}">{{ $source->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('lead_source_id') <span class="text-danger small">{{ $message }}</span> @enderror
                                </div>
                                <!-- Expected Date -->
                                <div class="col-md-4">
                                    <label for="expected_date" class="form-label fw-bold small">Expected Date</label>
                                    <input type="date" id="expected_date" wire:model="expected_date" class="form-control form-control-sm">
                                    @error('expected_date') <span class="text-danger small">{{ $message }}</span> @enderror
                                </div>
                                <!-- Stock Category Selection -->
                                <div class="col-md-4">
                                    <label for="category_id" class="form-label fw-bold small">Stock Category</label>
                                ...
                                <div class="col-md-4">
                                    <label for="child_category_id" class="form-label fw-bold small">Child Category</label>
                                    <select wire:model.live="child_category_id" id="child_category_id" class="form-select" {{ empty($childCategories) ? 'disabled' : '' }}>
                                ...
                                <div class="col-md-4">
                                    <label for="series" class="form-label fw-bold small">Series</label>
                                    <select id="series" wire:model="series" class="form-select form-select-sm">
                                        ...
                                        @else
                                            @foreach($seriesList as $singleSeries)
                                                <option value="{{ $singleSeries->id }}">{{ $singleSeries->name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    @error('series') <span class="text-danger small">{{ $message }}</span> @enderror
                                </div>
                            </div>

                            <!-- Form Actions -->
                            <div class="d-flex justify-content-between mt-3">
                                <button type="submit" class="btn btn-secondary btn-sm"><i class="ri-save-3-line"></i> Update</button>
                                @if(auth()->user()->hasRole('Agent'))
                                    <a href="{{ route('agent.leads.index') }}" class="btn btn-outline-danger btn-sm">
                                        <i class="bi bi-arrow-left me-1"></i> Back
                                    </a>
                                @endif
                            </div>
            ...
            <div class="col-md-12 ">
                <ul class="nav nav-tabs tab-style-2 nav-justified d-sm-flex d-block bg-white" id="myTab1" role="tablist">
                    ...
                    <li class="nav-item" role="presentation">
                        <button class="nav-link" id="shipped-tab" data-bs-toggle="tab"
                            ...
                            aria-controls="contact-tab-pane" aria-selected="false"><i class="ri-file-3-fill"></i>Files</button>
                    </li>
                </ul>
                <!-- Nav tabs -->
                <style>
                    .nav-tabs .nav-link.active {
                        background-color: #45D65B; /* Green background color */
                        color: white; /* Text color for better contrast */
                    }
                    .nav-tabs .nav-link {
                        color: #45D65B; /* Default text color for tabs */
                    }
                    .nav-tabs .nav-link:hover {
                        background-color: rgba(40, 217, 167, 0.1); /* Light green on hover */
                    }
                ...
                <div class="tab-content bg-white" id="myTabContent">
                    ...
                    aria-labelledby="home-tab" tabindex="0">
                        <div class="card  shadow-sm mb-4">
                            <div class="card-header bg-dark text-white py-2 d-flex justify-content-between align-items-center">
                                <h6 class="fw-bold"><span><i class="ri-information-line"></i> Lead Information :</span></h6>
                            ...
                            <div class="card-body">
                                ...
                                <!-- Lead Assigned To -->
                                <div class="mb-3">
                                    <h6 class="text-muted fw-bold"><i class="ri-user-settings-line me-2"></i> Team Assigned To:</h6>
                                    <p class="small">
                                        @if ($lead->assignedAgent)
                                            @if ($lead->assignedAgentTeams()->isNotEmpty())
                                                ...
                                                @endforeach
                                            @else
                                                <span class="text-secondary small">No teams assigned to the agent</span>
                                            @endif
                                        @else
                                            @if ($lead->user_id === Auth::id())
                                                ...
                                                @if ($lead->managerTeams()->isNotEmpty())
                                                    ...
                                                    @endforeach
                                                @else
                                                    <span class="text-secondary small">No teams assigned to the manager</span>
                                                @endif
                                            @else
                                ...
                                <!-- Lead Assign History -->
                                <div class="mb-3">
                                    <h6 class="text-muted fw-bold"><i class="ri-time-line me-2"></i>Assign History:</h6>
                                    <ul class="list-unstyled small">
                                        @forelse($lead->remarks as $remark)
                                            <li class="mb-1"><i class="ri-user-line me-1"></i>{{ $remark->user->name ?? 'Unknown' }} - <span class="text-muted">{{ $remark->created_at->format('d-m-Y H:i') }}</span></li>
                                        @empty
                    ...
                    <div class="tab-pane fade text-muted " id="shipped-tab-pane" role="tabpanel"
                        ...
                        <div class="card-body">
                            @if($remarks->isNotEmpty())
                            <h6 class="fw-bold"><span><i class="ri-chat-upload-line"></i> Uploaded Images :</span></h6>
                                <div class="row mt-2">
                                    @foreach($remarks->sortByDesc('created_at') as $remark) <!-- Sort remarks by creation date -->
                                        @if ($remark->image)
                                            <div class="col-md-3 mb-3">
                                                <div class="card shadow-sm d-flex justify-content-center align-items-center p-4">
                                                    ...
                                                    <div class="card-body text-center">
                                                        <h6 class="card-title">{{ $remark->user->name ?? 'Unknown User' }}</h6>
                                                        <p class="card-text text-muted">{{ $remark->created_at->format('d M Y') }}</p>
                                ...
                                <div wire:ignore.self id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true" class="modal fade">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-body text-center">
                                                <img id="modalImage" src="" alt="Remark Image" class="img-fluid rounded" style="max-width: 100%; height: auto;">
                                            </div>
                                            <div class="modal-footer justify-content-center">
                                                <button type="button" class="btn btn-dark rounded-pill" data-bs-dismiss="modal"><i class="ri-close-line"></i> Close</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
