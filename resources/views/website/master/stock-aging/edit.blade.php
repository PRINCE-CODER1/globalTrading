        ...
        <div class="col-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb breadcrumb-style2 mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Edit Stock Aging</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="row d-flex justify-content-center">
        <div class="container">
            <div class="col-12 mb-5 mt-3 bg-white p-5 shadow">         
                <form action="{{ route('stock-aging.update', $stockAging->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        ...
                        <select name="product_id" id="product_id" class="form-control" required>
                            @foreach($products as $product)
                                <option value="{{ $product->id }}" {{ $product->id == $stockAging->product_id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="quantity" id="quantity" class="form-control" value="{{ $stockAging->quantity }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="received_at" class="form-label fs-14 text-dark">Received At</label>
                        <input type="date" name="received_at" id="received_at" class="form-control" value="{{ $stockAging->received_at->format('Y-m-d') }}" required>
                    </div>
